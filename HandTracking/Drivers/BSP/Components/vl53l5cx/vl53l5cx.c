...
static int32_t vl53l5cx_get_result(VL53L5CX_Object_t *pObj, VL53L5CX_Result_t *pResult)
{
    ...
    // Controlla che ci siano dati pronti
    uint8_t ready = 0;
    if (vl53l5cx_check_data_ready(&pObj->Dev, &ready) != VL53L5CX_STATUS_OK || ready != 1) {
        printf("Frame non pronto\n");
        return VL53L5CX_ERROR;
    }

    printf("Frame pronto, leggo\n");

    // Leggi la risoluzione
    uint8_t resolution = 0;
    if (vl53l5cx_get_resolution(&pObj->Dev, &resolution) != VL53L5CX_STATUS_OK) {
        printf("Errore nel leggere la risoluzione\n");
        return VL53L5CX_ERROR;
    }

    if (resolution == 0) {
        printf("Risoluzione nulla\n");
        return VL53L5CX_ERROR;
    }

    printf("Risoluzione: %u\n", resolution);

    // Leggi i dati di ranging
    static VL53L5CX_ResultsData data;
    if (vl53l5cx_get_ranging_data(&pObj->Dev, &data) != VL53L5CX_STATUS_OK) {
        printf("Errore nel leggere i dati di ranging\n");
        return VL53L5CX_ERROR;
    }

    // Copia i dati nella struttura di output
    ...
    for (uint8_t i = 0; i < resolution; i++) {
        ...
        for (uint8_t j = 0; j < data.nb_target_detected[i]; j++) {
            uint16_t idx = (VL53L5CX_NB_TARGET_PER_ZONE * i) + j;

            pResult->ZoneResult[i].Distance[j] = (uint32_t)data.distance_mm[idx];
            pResult->ZoneResult[i].Status[j] =
                vl53l5cx_map_target_status(data.target_status[idx]);

            /*pResult->ZoneResult[i].Signal[j] =
                (pObj->IsSignalEnabled) ?
                (float_t)data.signal_per_spad[idx] : 0.0f;

            pResult->ZoneResult[i].Ambient[j] =
                (pObj->IsAmbientEnabled) ?
                (float_t)data.ambient_per_spad[i] : 0.0f;*/
        }
    }
